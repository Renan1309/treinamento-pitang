...
public interface ContentMenssageRepository extends JpaRepository<ContentMenssage, Long> {
    ...
    @Query(value = "select  * from content_menssage where   id_user_contact = ?1  and status_recipient = 'true' ", nativeQuery = true)
    List<ContentMenssage> findconversasEnviadasPorContatosNãoCadastrados(Long idusersend );
